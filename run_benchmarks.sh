...
do
    ...
    date;/usr/bin/time -v ./benchmarker_cgdrag_forpy ../cgdrag_model run_emulator_davenet      $nrun 10 --alloc_in_loop                    | tee cgdrag_forpy_$n.out;date
    date;/usr/bin/time -v ./benchmarker_cgdrag_torch ../cgdrag_model saved_cgdrag_model_cpu.pt $nrun 10 --alloc_in_loop --explicit_reshape | tee cgdrag_torch_explicit_$n.out;date
